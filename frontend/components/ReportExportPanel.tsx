...
export function ReportExportPanel({ analysisId }: { analysisId: string }) {
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
      <h3 className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Export report</h3>
      ...
      <div className="flex gap-2 flex-wrap">
        {FORMATS.map(({ format, label }) => (
          <a
            ...
            download
            className="rounded-lg border border-slate-300 dark:border-slate-700 text-sm font-medium px-4 py-1.5 hover:border-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
          >
            {label}
          </a>
        ))}
